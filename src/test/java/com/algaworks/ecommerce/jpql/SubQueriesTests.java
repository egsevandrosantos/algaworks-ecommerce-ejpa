...
public class SubQueriesTests extends EntityManagerTests {
	...
	@Test
	public void testEachProductSoldBySamePrice() {
		// String jpql = "SELECT p.name, oi.productPrice, p.price FROM OrderItem oi JOIN oi.product p";
		String jpql = """
			SELECT p.name, p.price
			FROM Product p
			WHERE
				...
				AND (SELECT MIN(oi1.productPrice) FROM OrderItem oi1 WHERE oi1.product = p) = ALL(SELECT oi1.productPrice FROM OrderItem oi1 WHERE oi1.product = p)
		""";
